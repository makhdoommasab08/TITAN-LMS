...
code = code.replace(signInFormStart, signInFormReplacement);
...
code = code.replace(signUpFormStart, signUpFormReplacement);

// Just to be sure we only replaced the first form, let's fix replacing form if it hit both forms. 
// wait, the string `<form onSubmit={handleSubmit} className="space-y-4">` might exist twice. 
// Let's modify the replace to only replace the first occurrence (sign in form).
